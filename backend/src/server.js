...
const kMessageRecallWindowMs = 2 * 60 * 1000;
const kRecalledSystemText = '\u4e00\u6761\u6d88\u606f\u5df2\u64a4\u56de';
const kTypingStateTtlMs = 6000;
const kMaxUploadBytes = 8 * 1024 * 1024;
const kUploadGcIntervalMs = 30 * 60 * 1000;
const kUploadGcGraceMs = 10 * 60 * 1000;
const kClientMessageIdMaxLength = 80;
...
async function ensureFriendRequestPairColumns() {
  if (!(await doesTableColumnExist('friend_requests', 'user_low_id'))) {
    await pool.query(
      `
        ...
        ADD COLUMN user_low_id BIGINT UNSIGNED NULL
      `
    );
  }
  ...
  if (!(await doesTableColumnExist('friend_requests', 'user_high_id'))) {
    await pool.query(
      `
        ALTER TABLE friend_requests
        ADD COLUMN user_high_id BIGINT UNSIGNED NULL
      `
    );
  }
  ...
  await pool.query(
    `
      ...
      SET
        user_low_id = LEAST(requester_user_id, addressee_user_id),
        user_high_id = GREATEST(requester_user_id, addressee_user_id)
  ...
  await pool.query(
    `
      ALTER TABLE friend_requests
      MODIFY COLUMN user_low_id BIGINT UNSIGNED NOT NULL,
      MODIFY COLUMN user_high_id BIGINT UNSIGNED NOT NULL
    `
  );
  ...
  if (!(await doesTableIndexExist('friend_requests', 'idx_friend_requests_pair_status'))) {
    await pool.query(
      `
        ALTER TABLE friend_requests
        ADD KEY idx_friend_requests_pair_status (user_low_id, user_high_id, status, id)
      `
    );
  }
  ...
  if (!(await doesForeignKeyExist('friend_requests', 'fk_friend_requests_user_low'))) {
    await pool.query(
      `
        ALTER TABLE friend_requests
        ADD CONSTRAINT fk_friend_requests_user_low
  ...
  if (!(await doesForeignKeyExist('friend_requests', 'fk_friend_requests_user_high'))) {
    await pool.query(
      `
        ALTER TABLE friend_requests
        ADD CONSTRAINT fk_friend_requests_user_high
...
async function ensureMessagesClientMessageIdSupport() {
  if (!(await doesTableColumnExist('messages', 'client_message_id'))) {
    await pool.query(
      `
        ...
        ADD COLUMN client_message_id VARCHAR(80) NULL AFTER sender_id
      `
    );
  }
  ...
  if (!(await doesTableIndexExist('messages', 'uk_messages_sender_client_message_id'))) {
    await pool.query(
      `
        ALTER TABLE messages
        ADD UNIQUE KEY uk_messages_sender_client_message_id
...
function createJwtToken(user, tokenJti) {
  return jwt.sign(
    {
      sub: String(user.id),
      email: user.email,
...
function sanitizeClientMessageId(rawValue) {
  ...
  return normalized.slice(0, kClientMessageIdMaxLength);
}
...
function canRecallMessageCreatedAt(createdAtValue, nowMs = Date.now(), windowMs = kMessageRecallWindowMs) {
...
function isLikelyUtf8Text(buffer) {
  ...
  for (const byte of sample) {
    ...
    if (isPrintableAscii || isCommonWhitespace || byte >= 0x80) {
      printableCount += 1;
    }
  }
...
function inferUploadMimeType(buffer, fileName, rawMimeType = '') {
  ...
  if (extensionMimeType === 'text/plain' && isLikelyUtf8Text(buffer)) {
    return extensionMimeType;
  }

  const claimedMimeType = normalizeMimeType(rawMimeType);
  if (claimedMimeType === 'text/plain' && isLikelyUtf8Text(buffer)) {
    return 'text/plain';
  }
...
function sendServerError(res, message, error) {
  ...
  if (config.exposeErrorDetail && error && String(error.message || '').trim()) {
    payload.detail = String(error.message || '').trim();
  }
  return res.status(500).json(payload);
}

async function readRequestBody(req, maxBytes = kMaxUploadBytes + (1024 * 64)) {
...
async function parseMultipartFormData(contentType, bodyBuffer) {
  if (typeof Request !== 'function') {
    throw new Error('Multipart parser is unavailable in current Node runtime.');
  }

  const header = String(contentType || '').trim();
  const match = header.match(/boundary=(?:"([^"]+)"|([^;]+))/i);
  const boundary = match ? String(match[1] || match[2] || '').trim() : '';
  if (!boundary) {
  ...
  try {
    ...
    throw new Error(`Invalid multipart payload: ${String(error.message || '').trim() || 'parse failed.'}`);
  }
  ...
  for (const [fieldName, value] of formData.entries()) {
    ...
    if (!filePart && value && typeof value.arrayBuffer === 'function') {
...
async function extractUploadPayloadFromRequest(req) {
  const rawContentType = String(req.headers['content-type'] || '').trim();
  const normalizedContentType = rawContentType.toLowerCase();
  if (normalizedContentType.startsWith('multipart/form-data')) {
    const bodyBuffer = await readRequestBody(req, kMaxUploadBytes + (1024 * 512));
    const { fields, filePart } = await parseMultipartFormData(rawContentType, bodyBuffer);
    if (!filePart || !filePart.buffer) {
      throw new Error('Missing file part.');
    }

    return {
      fileName: sanitizeUploadFileName(filePart.fileName || fields.get('fileName')),
      mimeType: String(filePart.mimeType || fields.get('mimeType') || 'application/octet-stream')
        .trim()
        .toLowerCase(),
      clientMessageId: String(fields.get('clientMessageId') || '').trim(),
